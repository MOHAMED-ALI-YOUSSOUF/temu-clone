...
const Cart = () => {
    ...
    const handleProceedToCheckout = () => {
        ...
        const phoneNumber = "+905314340471"; // Remplace avec ton numéro WhatsApp
  let message = "🛒 *Nouvelle commande :*\n\n";

  items.forEach((item, index) => {
    message += `🔹 *${item.name}*\n`;
    message += `🖼️ Image : ${item.imageUrl}\n`; // Lien vers l'image
    message += `💰 Prix : ${item.price} €\n`;
    message += `🔢 Quantité : ${item.quantity}\n`;
    message += `🏷️ Total : ${item.price * item.quantity} €\n`;
    message += `-----------------\n`;
  });

  const totalPrice = items.reduce((total, item) => total + item.price * item.quantity, 0);
  message += `💵 *Total général :* ${totalPrice} €\n\n`;
  message += "📍 *Adresse de livraison :* (À remplir par le client)\n";
  message += "📞 *Numéro de contact :* (À remplir par le client)\n";
